...
class RecipeState {
  constructor() {
    ...
    this.source = "";
    this.error = null;
    this.dietaryPreferences = []; // Added dietary preferences
  }
}
...
async function searchDatabase(state) {
  ...
  try {
    ...
    // Build search query with dietary preferences filter if provided
    const baseQuery = state.dietaryPreferences && state.dietaryPreferences.length > 0
      ? { dietType: { $in: state.dietaryPreferences } }
      : {};
...
async function callGeminiAPI(state) {
  ...
  try {
    ...
    const dietaryInstruction = state.dietaryPreferences && state.dietaryPreferences.length > 0
      ? `Make sure the recipe is suitable for someone with these dietary preferences: ${state.dietaryPreferences.join(', ')}.`
      : "";
...

{
  ...
  "calories": 500
    ...
    console.log("[Gemini Node] ✅ Recipe saved to database");
    return {
      ...state,
      recipes: [savedRecipe],
      source: "gemini",
...
function routeDecision(state) {
  if (state.error) {
    console.log("[Router] Error detected, ending workflow");
    return END;
  }

  if (state.source === "database") {
    console.log("[Router] Recipe found in database, ending workflow");
    return END;
  }

  if (state.source === "none") {
    console.log("[Router] No recipe in database, routing to Gemini");
    return "gemini";
  }
