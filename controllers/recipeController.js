...
// Helper: Call Gemini API optimized for Vercel
async function callGeminiAPI(prompt) {
    try {
        ...
        // Vercel has 10s timeout on Hobby plan, so we need to be faster
        const response = await axios.post(
            ...
            },
            {
                ...
                timeout: 8000 // 8 second timeout for Vercel compatibility
            }
        );
        ...
        if (candidate.finishReason && candidate.finishReason !== 'STOP') {
            ...
            throw new Error(`Gemini API error: ${candidate.finishReason}`);
        }

        let content = candidate.content.parts[0].text;
        console.log('Raw Gemini content:', content.substring(0, 200) + '...');
...
Return ONLY valid JSON in this exact format (no markdown, no code blocks):
{
  ...
  "allergens": ["nuts", "dairy"]
        ...
        const geminiRecipe = await callGeminiAPI(prompt);
        console.log('📥 Received from Gemini:', geminiRecipe.name);
        ...
        res.status(500).json({ 
            error: error.message,
            details: 'Failed to generate recipe. Please check server logs for details.'
        });
    }
};
    ...
    try {
        ...
        const recipes = await Promise.all(
            recipeNames.map(async (name) => {
                ...
                if (!recipe) {
                    console.log(`⚡ Fetching ${name} from Gemini...`);
